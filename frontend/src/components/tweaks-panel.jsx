...
export function TweaksPanel({ title = TITLE, children }) {
  ...
  return (
    <>
      ...
      <div ref={dragRef} className="twk-panel" data-noncommentable=""
        ...
        <div className="twk-hd" onMouseDown={(e) => {
...
export function Radio({ label, value, options, onChange }) {
  ...
  if (!fitsAsSegments) {
    ...
    return <Select label={label} value={value} options={options} onChange={(s) => onChange(resolve(s))} />;
  }
...
export function TweakRadio({ label, value, options, onChange }) {
  return <Radio label={label} value={value} options={options} onChange={onChange} />;
}
        ...
        {options.map((o, i) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio" aria-checked={on}
              ...
              {rest.length > 0 && (
                <span>{rest.map((c, j) => <i key={j} style={{ background: c }} />)}</span>
              )}
